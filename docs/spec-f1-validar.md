...
| Fase | Faz |
|---|---|
| `implement` | Lê o contrato, registra identidade (bytes + `sha256`) e gera `evidence/<run_id>/f1-relatorio.html` via `datacontract export html`. |
| `verify` | Roda `datacontract lint --output-format json` gravando `evidence/<run_id>/f1-lint.json`, lê o veredito e devolve PASS/FAIL. |

`implement` **produz**, `verify` **julga**. A separação não é estética: é o que
faz `./run.sh verify` valer sozinho. A fase reexecuta a validação inteira sem
